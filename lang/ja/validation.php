...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | `app/Rules/` のルールが文言キーを指定されなかった場合に使う。
    |
    | `custom` 側（属性名 × ルール名）に置かない。`custom.name.regex` のような
    | 一般的な属性名のキーは、将来 `name` に `regex` を課した無関係の画面へ
    | 「氏名は全角文字で…」を出してしまう。
    |
    */

    'rules' => [
        'full_width_text' => ':attributeは全角文字で入力してください。',
        'full_width_katakana' => ':attributeは全角カタカナで入力してください。',
    ],
